...
def file_filter(name):
    return (not name.startswith(".") and not name.endswith(".swp") and
            not name.endswith(".pyc") and not name.endswith(".log") and
            not name == "migrations" and not name == "test_scripts")
    ...
    for top_level in filter(file_filter, os.listdir(path)):
        for root, dirs, files in os.walk(top_level):
            for file in filter(file_filter, files):
                yield (os.stat(os.path.join(root, file)).st_mtime, os.path.join(root, file))
